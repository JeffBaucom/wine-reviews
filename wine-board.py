...
class WineBoard(object):
    ...
    def calculator_helper(self, term, category):
        ...
        Takes a term and a category to compare it
        returns the distance to that category
    ...
    def comb_categories(self):
        """
        Combs the vocab generated by WineDictionary for words most similar to 
        the words in the given_categories

        Adds terms to added_categories (dictionary keyed with categories and
        which store arrays of words for each category)
        uses wordlist to check for duplicates (flat array)
        added_terms dictionary provides multiclass word functionality
    ...
    def print_results(self):
        """
        Intended for printing the words most recently added to added_categories
        """
    ...
    def calculate_distances(self):
        """
        builds x_train and y_train from added_terms
    ...
    def write_vectors(self):
        """
        write n-dimensional word vectors for each word
    ...
    def train_words(self, n):
        """
        n - number of passes
        driver function for word training
        performs combing, calculates distances
        transforms training data with WordClassifier
        fits training data with WordClassifier
        """
        for i in range(n):
    ...
    def train_wines(self, n):
        """
        n - number of wines total (split 1/2 and 1/2 for train and test)
        driver function for wine training
        splits wine training data, fits WineClassifier
        currently runs prediction as well
        """
